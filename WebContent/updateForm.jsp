...
<form action="updatePro.do?sid=${sid }&qid=${qid}&qctg=${qctg}" method="post" enctype="multipart/form-data">
   <input type="hidden" name="qctg" value="${qna.qctg }">
   <input type="hidden" name="qid" value="${qna.qid }">
   <input type="hidden" name="sid" value="${qna.sid }">
   <input type="hidden" name="pageNum" value="${pageNum }">
   <table>
      <caption><h2>게시판 수정</h2></caption>
      <tr><td class="update">유저번호</td><td class="udapte2">${qna.sid }번</td></tr>
      <tr><td class="update">제목</td><td>문의드립니다~★</td></tr>
      <tr><td class="update">작성자</td><td>${ sname} </td></tr>
      <tr><td class="update">파일</td><td><input type="file" name="qfile"></td></tr>
      <tr><td class="update">내용</td><td><pre><textarea rows="10" cols="40" name="qcontent"
      required="required">${qna.qcontent }</textarea></pre></td></tr>
      <tr><td colspan="2"><input type="submit" value="수정완료" class="submit">
   </table>
</form>
</div>  
